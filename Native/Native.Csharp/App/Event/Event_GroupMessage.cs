...
namespace Native.Csharp.App.Event
{
    public class Event_GroupMessage : IEvent_GroupMessage
    {
        ...
        //private static  IRepository<GroupMessage> repository = Event_AppMain.ServiceProvider.GetService<IRepository<GroupMessage>>();

        public Event_GroupMessage(IRepository<GroupMessage> repository, ICommandService service)
        {
        ...
        public void ReceiveGroupMessage(object sender, GroupMessageEventArgs e)
        {
            ...
            try
            {
                ...
                // repository = Event_AppMain.ServiceProvider.GetService<IRepository<GroupMessage>>();
                // 于2019年02月26日, 默认注释此行代码.
                repository.Add(new GroupMessage()
                {
                ...
                if (service.IsCommand(e.Msg, out string commandName))
                {
                    var cmd = new Command()
                    {
                    ...
                    if (service.Excuter(cmd, out string message))
                    {
                ...
                Debug.WriteLine(Common.CqApi.CqCode_At(e.FromQQ) + "你发送了这样的消息: " + e.Msg);
            }
            catch (Exception ex)
            {
                Common.CqApi.AddLoger(Sdk.Cqp.Enum.LogerLevel.Error, "", ex.Message);
